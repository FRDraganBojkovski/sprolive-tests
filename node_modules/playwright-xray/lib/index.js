...
class XrayReporter {
    ...
    async onTestEnd(testCase, result) {
        ...
        if (testCodes !== "") {
            ...
            switch (this.help.convertPwStatusToXray(result.status)) {
                ...
                case "PASSED":
                    if (result.retry > 0)
                        console.log(`${(0, picocolors_1.bold)((0, picocolors_1.yellow)(`⚠️  ${projectID}${testCase.title}`))}`);
                    else
    ...
    async onEnd(result) {
        ...
        this.testResults.info.finishDate = this.help.getFormatData(new Date(new Date(this.testResults?.info?.startDate ?? new Date()).getTime() + result.duration));
